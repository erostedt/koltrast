...
[[nodiscard]] constexpr inline RGB<f32> directional_light(const Vec3f &world_position, const Vec3f &world_normal,
                                                          const Vec3f &light_direction, const Vec3f &camera_position,
                                                          const RGB<f32> &light_color, float specular, float shininess)
{
    ...
    const float spec = std::pow(std::max(view_direction.dot(reflection_direction), 0.0f), shininess);
...
[[nodiscard]] constexpr inline RGB<f32> point_light(const Vec3f &world_position, const Vec3f &world_normal,
                                                    const Vec3f &light_position, const Vec3f &camera_position,
                                                    const RGB<f32> &light_color, float specular, float shininess)
{
    const Vec3f light_direction = *(light_position - world_position).normalized();
    return directional_light(world_position, world_normal, light_direction, camera_position, light_color, specular,
                             shininess);
}
