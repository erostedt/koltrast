...
template <std::floating_point T, size_t TileRows = 8, size_t TileCols = TileRows, size_t AARows = 1,
          size_t AACols = AARows>
    requires(TileRows > 0) && (TileCols > 0) && (AARows > 0) && (AACols > 0)
...
template <std::floating_point T, FragmentShader<T> FragmentShader, size_t AARows = 1, size_t AACols = AARows>
    requires(AARows > 0) && (AACols > 0)
inline void render_vertices(const std::vector<Vec3<T>> &screen_coordinates,
                            ...
                            const IndexBuffer<AARows, AACols> &index_buffer, ColorImage<T> &linear_image) noexcept
{
    static constexpr Matrix<Vec2<T>, AARows, AACols> offsets = make_aa_grid<T, AARows, AACols>();
    using namespace std;
...
template <std::floating_point T, VertexShader<T> VertexShader, FragmentShader<T> FragmentShader, size_t TileRows = 8,
          size_t TileCols = TileRows, size_t AARows = 1, size_t AACols = AARows>
    requires(AARows > 0) && (AACols > 0)
constexpr inline void render(const std::vector<Face> &faces, const std::vector<Vec3<T>> &positions,
                             ...
                             IndexBuffer<AARows, AACols> &index_buffer, std::vector<OutputVertex<T>> &vertex_buffer,
                             std::vector<Vec3<T>> &screen_coordinates, DepthBuffer<T, AARows, AACols> &depth_buffer,
                             ColorImage<T> &linear_image) noexcept
{
    ...
    rasterize_triangles<T, TileRows, TileCols, AARows, AACols>(screen_coordinates, depth_buffer, index_buffer);
    render_vertices(screen_coordinates, vertex_buffer, fragment_shader, index_buffer, linear_image);
}
...
template <std::floating_point T, size_t AARows = 1, size_t AACols = AARows> class Renderer
{
  public:
    ...
    {
        ::render(faces, positions, normals, texture_coordinates, vertex_shader, fragment_shader, index_buffer,
                 vertex_buffer, screen_coordinates, depth_buffer, linear_image);
    }
